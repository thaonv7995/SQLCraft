/**
 * MySQL EXPLAIN FORMAT=JSON → Postgres-shaped plan (same tree UI as Postgres).
 * Logic mirrors `packages/mysql-explain`; change both when updating the worker adapter.
 */
...
/** Merge query_block-level cost/rows onto root (see packages/mysql-explain). */
